...
async function consortium<T>(
  ...
  await Promise.all(Array.from({ length: size }, async (_, i) => {
    ...
    emit({ type: 'node_done', nodeId: aid, success: parsed !== null, summary: String(lastText || '')?.slice(0, 80), rawResponse: lastText, durationMs: Date.now() - startAid });
...
async function runErrorCorrection(
  ...
  const efid = nid('jury');
  const efUserMsg = `Node name: ${name}\nDepth: ${depth}\nProblem: ${String(context || '')?.slice(0, 400)}\nExecution result: ${String(previousAnswer || '')?.slice(0, 300)}\nFailure analysis: ${String(failureSummary || '')?.slice(0, 300)}\n\n${isRoot ? 'This is the ROOT node — answer YES.' : 'Is this node responsible for the failure?'}`;
...
export async function execRafNode(
  ...
  if (isBase) {
    ...
    emit({ type: 'node_done', nodeId: eid, success: true, summary: String(execR.text || '')?.slice(0, 100), rawResponse: execR.text, durationMs: Date.now() - startEid });

    const answerMatch = execR.text.match(/####\s*(.+)$/m);
    const answer = answerMatch?.[1]?.trim() ?? String(execR.text || '')?.slice(-200).trim();
